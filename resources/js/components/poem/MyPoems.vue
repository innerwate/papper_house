<template>
  <div class="row">
    <div class="poems col-12">
      ...
      <categories-menu
        @update-poems="poemsByCategories"
        :isUsers="true"
        :user_id="_user.id"
      ></categories-menu>
...
export default {
  ...
  created() {
    ...
    if (this._user) {
      this.getMyPoems(this._user.id);
    }
  },
  methods: {
    ...mapActions("poem", [ "getPoemsByCategories", "getMyPoems"]),
    ...mapGetters("poem", ["poemsList", "myPoems"]),
    ...
    poemsByCategories(categoriesArray) {
      let data = { categoriesArray: categoriesArray, isUsers: true , user_id: this._user.id};
